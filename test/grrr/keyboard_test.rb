class TestKeyboard < Test::Unit::TestCase
	...
	test "a keyboard should by default be 7x2, coupled, have basenote 60 and indicate both black and white keys" do
		keyboard = Grrr::Keyboard.new_detached
	...
	test "it should be possible to create a decoupled keyboard" do
		assert_nothing_raised {
			Grrr::Keyboard.new_decoupled(nil, nil, 7, 72)
		}
	end
...
=begin
	test "it should only be possible to create a keyboard with a basenote that is a white key on the keyboard" do
		assert_nothing_raised {
			Grrr::Keyboard.new_detached(7, 72)
		}
		assert_raise(RuntimeError) {
			Grrr::Keyboard.new_detached(7, 73)
		}
	end
=end

	test "it should be possible to change basenote of a keyboard" do
		keyboard = Grrr::Keyboard.new_detached(7, 60)
...
=end
	...
	test "when basenote property is changed pressed buttons on keyboard view should be released" do
		keyboard = Grrr::Keyboard.new_detached(7, 60)
		keyboard.press(Grrr::Point.new(0, 1))
		keyboard.press(Grrr::Point.new(2, 0))
		keyboard.press(Grrr::Point.new(4, 1))
...
=begin
	test "when basenote is changed the keyboard should automatically refresh" do
		...
		assert(
			view_led_refreshed_listener.has_been_notified_of?(
				[
					{ :source => :keyboard, :point => Grrr::Point.new(0, 0), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(1, 0), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(2, 0), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(3, 0), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(4, 0), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(5, 0), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(6, 0), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(0, 1), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(1, 1), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(2, 1), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(3, 1), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(4, 1), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(5, 1), :on => true },
					{ :source => :keyboard, :point => Grrr::Point.new(6, 1), :on => true },
				]
			)
		)
	end
	...
	test "it should be possible to retrieve the keyrange of a keyboard" do
		keyboard = Grrr::Keyboard.new_detached(3, 62)
		assert_equal(
			(61..65),
			keyboard.keyrange
		)
	...
	test "it should be possible to retrieve the number of notes in the keyrange of a keyboard" do
		keyboard = Grrr::Keyboard.new_detached(4, 60)
	...
	test "the value of a keyboard should be an array of boolean values corresponding to how notes are displayed" do
		...
		keyboard.press(Grrr::Point.new(2, 1))
		keyboard.press(Grrr::Point.new(3, 0))
	...
	test "when a note is displayed as pressed on a keyboard its corresponding led should be inverted from its normal led state" do
		...
		assert_equal(
			true,
			keyboard.is_unlit_at?(Grrr::Point.new(2, 1))
		)
	end
	...
	test "when a button on the keyboard view that corresponds to a note that currently is pressed is released the state should be reflected" do
		...
		keyboard.release(Grrr::Point.new(2, 1))
	...
	test "it should be possible to get notified of notes being pressed on view by adding an action to a keyboard" do
		...
		keyboard.press(Grrr::Point.new(2, 1))
		keyboard.press(Grrr::Point.new(4, 1))
		keyboard.press(Grrr::Point.new(1, 0))
		keyboard.release(Grrr::Point.new(4, 1))
		keyboard.release(Grrr::Point.new(1, 0))
		keyboard.release(Grrr::Point.new(2, 1))
	...
	test "it should be possible to display notes as pressed on a decoupled keyboard" do
		keyboard = Grrr::Keyboard.new_decoupled(nil, nil, 7, 60)
	...
	test "when indicate keys property is changed the keyboard should automatically refresh" do
		...
		assert(
			view_led_refreshed_listener.has_been_notified_of?(
				[
					{ :source => :keyboard, :point => Grrr::Point.new(0, 0), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(1, 0), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(2, 0), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(3, 0), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(4, 0), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(5, 0), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(6, 0), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(0, 1), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(1, 1), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(2, 1), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(3, 1), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(4, 1), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(5, 1), :on => false },
					{ :source => :keyboard, :point => Grrr::Point.new(6, 1), :on => false },
				]
			)
		)
	end
