class TestView < Test::Unit::TestCase
	...
	test "it should be possible to enable disabled views" do
		view = View.new_disabled(nil, nil, 4, 4)
		view.enable
		assert(view.is_enabled?)
	end
	...
	test "it should be possible to retrieve points of a view" do
		assert_equal(
			...
			View.new_detached(2, 3).to_points
		)
	end
	...
	test "it should be possible to retrieve points of a view starting from an origin" do
		assert_equal(
			...
			View.new_detached(2, 3).to_points_from(Point.new(10, 20))
		)
	end
	...
	test "it should be possible to retrieve the left top, right top, left bottom and right bottom points of a view" do
		assert_equal(
			Point.new(0, 0),
			View.new_detached(2, 3).left_top_point
		)
		assert_equal(
			Point.new(1, 0),
			View.new_detached(2, 3).right_top_point
		)
		assert_equal(
			Point.new(0, 2),
			View.new_detached(2, 3).left_bottom_point
		)
		assert_equal(
			Point.new(1, 2),
			View.new_detached(2, 3).right_bottom_point
		)
	end
	...
	test "refreshing a disabled view should throw an error" do
		view = View.new_disabled(nil, nil, 2, 2)
